...
BEGIN
	...
	SELECT 
		...
		SUBSTRING(prd_key, 7, LENGTH(prd_key)) AS prd_key, --length returns the # of characters in a string 
		prd_nm,
		COALESCE(prd_cost, 0) AS prd_cost,  --COALESCE or Isnull replaces null values with a specified replacement value
	...
	SELECT
		...
		CASE 
			...
			ELSE CAST(CAST(sls_order_dt AS VARCHAR) AS DATE) --changing order date(integer) to date
		END AS sls_order_dt,
		CASE 
	...
	SELECT 
		CASE 
			WHEN cid LIKE 'NAS%' THEN SUBSTRING(cid, 4, LENGTH(cid)) --length is used to not restrict length
			ELSE cid
